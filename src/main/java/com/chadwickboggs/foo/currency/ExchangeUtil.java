...
//
// IMPORTANT NOTE:
//    I did only the scaffolding believing it would reveal more details than did the algorithm.
//
...
public final class ExchangeUtil {
  ...
  public static ExchangePath cheapestExchangePath(
      @NotNull final String fromCurrency, @NotNull final String toCurrency ) {
    //
    // Design Note: Guava preconditions possibly preferable over assert statements
    // as assertions may be disabled.
    //
  ...
  //
  // Design Note:
  //    This is one point where the scaffolding ends and the algorithm begins.
  //    It must detect avoid infinite loops.  One idea would be for every possible path between
  //    every node to be pre-calculated and cached and/or persisted, then this method would be a
  //    lookup.
  //
  private static List<ExchangePath> everyExchangePath(
  ...
  private static final class DelimitedStringUtil {
    ...
    public static String toString(
        final List<String> everyPossiblePath, final boolean ignoreEmptyValues
    ) {
  ...
  public class ExchangeRate {
    ...
    public ExchangeRate(
        final String fromCurrency, final String toCurrency, final float exchangeRate
    ) {
